...
export async function createReunion(req, res) {
  try {
    ...
    const esDuplicada = await validarReunionPorFecha(fecha);
    if (esDuplicada) {
      console.warn("⚠️ Ya hay una reunión con la misma fecha y hora.");
      return handleErrorClient(
        res,
        400,
        "❌ Ya hay una reunión registrada en ese horario."
      );
    }
...
export async function eliminarActa(req, res) {
  try {
    ...
    const [mensaje, error] = await eliminarActaService(id);
    ...
    console.log("🗑️ Acta eliminada correctamente");
    handleSuccess(res, 200, mensaje);
  } catch (error) {
    console.error("❌ Error interno al eliminar acta:", error);
    handleErrorServer(res, 500, error.message);
  }
}
